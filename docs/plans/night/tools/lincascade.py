...
work=[dict(zeros=set(), det={}, free=set(), C=[], N=[], rem=polys, hist=[])]
results=[]; t0=time.time(); nb=0
while work:
    st=work.pop(); nb+=1
    ...
    if not rem:
        results.append(dict(hist=st['hist'], result='END', free=sorted(st['free']), nC=len(st['C']), nN=len(st['N']), C=[str(c) for c in st['C']], N=[str(n) for n in st['N']], det={k:(str(v.n),str(v.d)) for k,v in st['det'].items()})); continue
    ...
    if newv and dmax>1:
        ...
        st2=dict(st); st2['zeros']=st['zeros']|nil; st2['free']=st['free']|(s-nil); st2['C']=st['C']+rest; st2['rem']=newrem; st2['hist']=st['hist']+[f'{v}=0' for v in sorted(nil)]
    ...
    for (i,col,pv) in alt:
        st2=dict(st); st2['C']=st['C']+[pv]; st2['hist']=st['hist']+[f'piv0:{col}']
        # in the zero branch, that row's entry is zero: encode by adding pv to C and replacing the generator set with rows? simpler: keep rem, add constraint; the same pivot will be chosen again -> avoid by recording
        st2['avoid']=st.get('avoid',set())|{str(pv)}
        work.append(st2)
    # back-substitution
    ...
    work.append(dict(zeros=st['zeros'], det=det, free=free, C=C+newC, N=N, rem=newrem, hist=st['hist']+[f'stage{sorted(cols)}'], avoid=st.get('avoid',set())))
...
for r in results:
    if r['result']=='END': print('END branch', r['hist'], 'free', r['free'], 'C', r['nC'], 'N', r['nN'], 'Cdegs', [len(c) for c in r['C']][:10])
