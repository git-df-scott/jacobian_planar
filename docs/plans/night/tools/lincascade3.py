...
while work:
    ...
    if newv and dmax>1:
        # general nonlinear block: substitute determined rationals, clear denominators, GB in free+new vars
        blk2=[]
        for f in blk:
            r=eval_rat(f, st['det']); blk2.append(r.n)
        allv=sorted(set(v for f in blk2 for v in vars_of(f)))
        B=msolve_gb(blk2+[c for c in st['C'] if vars_of(c)<=set(allv)], allv, f'nl{nb}')
        ...
        monos=[vars_of(b) for b in B if len(b.monoms())==1]
        if monos:
            mv=min(monos, key=len)-st['nz']
            if not mv: results.append(dict(hist=st['hist'], result='EMPTY (monomial GB element in nonzero variables)')); continue
            push_zero_branches(st, mv, 'nonlinear-gb'); continue
        st2=dict(st); st2['free']=st['free']|newv; st2['C']=st['C']+B; st2['rem']=newrem; st2['hist']=st['hist']+[f'nl{sorted(newv)}']
        print(f"[{nb}] nonlinear block new {sorted(newv)} deg {dmax}: GB {len(B)} added as constraints, free {sorted(st2['free'])}", flush=True)
