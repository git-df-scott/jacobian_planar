"""night16 -- exact mate solve (night15 module, copied into this lane) for the survivors of the period screen.
...
import matekit16 as M        # copy of night12/matekit.py, unchanged
import exact16 as EX         # copy of night12/exact.py, unchanged but for its
                             # import of matekit
import pk16 as P14
...
def solve(P, max_cols=2600, verbose=True, degs=None, lam_cap=900):
    ...
    stages = []
    for D in (degs if degs is not None else (d, (3 * d + 1) // 2, 2 * d)):
        S = carrier(D)
        if len(S) > max_cols:
