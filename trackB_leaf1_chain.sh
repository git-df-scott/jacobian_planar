...
rc=$?   # capture BEFORE any command substitution, which would reset $?
echo "$(date +%H:%M:%S) chain: p65539 exit=$rc" >> trackB_leaf1_chain.log

JCP=65599 python3 trackB_leaf1_sweep.py >> trackB_leaf1_chain.log 2>&1
rc=$?
echo "$(date +%H:%M:%S) chain: p65599 exit=$rc" >> trackB_leaf1_chain.log
...
rc=$?
echo "$(date +%H:%M:%S) chain: leaf-1 exact-Q exit=$rc" >> trackB_leaf1_chain.log
