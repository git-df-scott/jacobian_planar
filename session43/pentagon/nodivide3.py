...
for b in range(0, 13): G[b] = sum(sp.Symbol(f'g{b}_{k}')*z**k for k in gsup(b))
def lev(L):
    e = 0
    for a in range(-1, 9):
        b = L-a
...
def newsyms(L):
    ...
    if 0 <= L-8 <= 12: out += [sp.Symbol(f'g{L-8}_{k}') for k in gsup(L-8)]
    return out
sub, conds = {}, []
        ...
        for q in list(left):
            ...
            for u in targets:
                for x in sorted(u.free_symbols, key=str):
                    ...
                    setsub(x, val)
                    left = [sp.expand(w.subs(x, val)) for w in left if w is not q]
                    left = [w for w in left if sp.expand(w) != 0]
...
pickle.dump((conds, sub), open('nodivide3.pkl','wb'))
print("saved nodivide3.pkl", flush=True)
