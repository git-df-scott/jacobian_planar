...
# -*- coding: utf-8 -*-
"""
Dimension 3 Keller counterexample of GEOMETRIC DEGREE 6  (det J = 2)

A non-injective Keller map C^3 -> C^3 whose generic fibre has SIX points.
Built by running Gao's tangent-sweep + monomial-twist scheme (arXiv:2608.00222,
sec. 3.3) on the degree-5 plane curve p(w) = -w^5+5w^4-8w^3+w^2+4w.
Geometric degree = deg(p)+1 = 6, i.e. above the range 2..5 that is excluded
for PLANAR Keller maps (Orevkov 1986 rules out planar geometric degree 3).
...
Source of the construction:
  ...
  The particular degree-5 curve below was chosen HERE, not copied from a
  paper.  The paper's "side conditions" for the twist were re-derived from
  scratch and validated against the paper's own two worked examples
  (Alpoge's d=2 map and Gao's d=3 map G) before being solved for deg(p)=5;
  see sweep_lib.py and alpoge_dim3_degree3.py / gao_G_dim3_degree4.py.

Run:  python3 dim3_degree6.py
...
F2 = (
    (-1)*x**12*y**5*z**4 + (-12)*x**11*y**6*z**3 + (-54)*x**10*y**7*z**2 + (-5)*x**11*y**4*z**4
    + (-108)*x**9*y**8*z + (-52)*x**10*y**5*z**3 + (-81)*x**8*y**9 + (-198)*x**9*y**6*z**2
    + (-10)*x**10*y**3*z**4 + (-324)*x**8*y**7*z + (-85)*x**9*y**4*z**3 + (-189)*x**7*y**8
    + (-249)*x**8*y**5*z**2 + (-10)*x**9*y**2*z**4 + (-279)*x**7*y**6*z + (-60)*x**8*y**3*z**3
    + (-81)*x**6*y**7 + (-90)*x**7*y**4*z**2 + (-5)*x**8*y*z**4 + (32)*x**6*y**5*z
    + (-10)*x**7*y**2*z**3 + (96)*x**5*y**6 + (52)*x**6*y**3*z**2 + (-1)*x**7*z**4
    + (142)*x**5*y**4*z + (8)*x**6*y*z**3 + (77)*x**4*y**5 + (42)*x**5*y**2*z**2
    + (40)*x**4*y**3*z + (3)*x**5*z**3 + (-1)*x**3*y**4 + (3)*x**4*y*z**2 + (-8)*x**3*y**2*z
    + (-2)*x**2*y**3 + (-2)*x**3*z**2 + (2)*x**2*y*z + (10)*x*y**2 + (3)*x*z + (1)*y
)

F3 = (
    (R(-5, 12))*x**12*y**6*z**4 + (-5)*x**11*y**7*z**3 + (R(-45, 2))*x**10*y**8*z**2
    + (R(-5, 2))*x**11*y**5*z**4 + (-45)*x**9*y**9*z + (R(-80, 3))*x**10*y**6*z**3
    + (R(-135, 4))*x**8*y**10 + (-105)*x**9*y**7*z**2 + (R(-25, 4))*x**10*y**4*z**4
    + (-180)*x**8*y**8*z + (-57)*x**9*y**5*z**3 + (R(-225, 2))*x**7*y**9
    + (R(-371, 2))*x**8*y**6*z**2 + (R(-25, 3))*x**9*y**3*z**4 + (-249)*x**7*y**7*z
    + (-60)*x**8*y**4*z**3 + (R(-441, 4))*x**6*y**8 + (-138)*x**7*y**5*z**2
    + (R(-25, 4))*x**8*y**2*z**4 + (R(-284, 3))*x**6*y**6*z + (R(-85, 3))*x**7*y**3*z**3
    + (13)*x**5*y**7 + (R(-21, 2))*x**6*y**4*z**2 + (R(-5, 2))*x**7*y*z**4 + (83)*x**5*y**5*z
    + (R(937, 12))*x**4*y**6 + (43)*x**5*y**3*z**2 + (R(-5, 12))*x**6*z**4 + (80)*x**4*y**4*z
    + (5)*x**5*y*z**3 + (R(63, 2))*x**3*y**5 + (R(39, 2))*x**4*y**2*z**2
    + (R(34, 3))*x**3*y**3*z + (R(4, 3))*x**4*z**3 + (R(-19, 4))*x**2*y**4 + (-5)*x**2*y**2*z
    + (R(1, 3))*x*y**3 + (-1)*x**2*z**2 + (1)*x*y*z + (3)*y**2 + (1)*z
)

F = [F1, F2, F3]

# --- generating data (provenance; see module docstring) --------------------
P_OF_W = (-1)*w**5 + (5)*w**4 + (-8)*w**3 + (1)*w**2 + (4)*w**1
Q_OF_W = (R(-5, 12))*w**6 + (2)*w**5 + (-3)*w**4 + (R(1, 3))*w**3 + (1)*w**2          # q(w) = int_0^w (s/2) p'(s) ds
G0 = 2
A  = -3
B  = -1
CLAIMED_DEGREE = 6          # = deg(p) + 1
CLAIMED_DETJ   = 2
