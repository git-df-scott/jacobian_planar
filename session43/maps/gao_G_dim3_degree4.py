...
# -*- coding: utf-8 -*-
"""
Gao's map G: dimension 3, geometric degree 4  (det J = 2)

Reference map from Gao arXiv:2608.00222, Theorem 3.5 (component degrees
4, 11, 12; det J = 2; generic fibre 4 points).
NOTE: geometric degree 4 -- still below the 6 you need.
...
Source of the construction:
  ...
  p, q, gamma, u, w are exactly as printed in Gao sec. 3.5:
     p(w) = w^3-6w^2+6w,   q(w) = (3/8)w^4-2w^3+(3/2)w^2,
     gamma = 2-4xy-x^2 z,  u = 1+xy,  w = gamma*u.

Run:  python3 gao_G_dim3_degree4.py
...
F2 = (
    (1)*x**6*y**3*z**2 + (8)*x**5*y**4*z + (16)*x**4*y**5 + (3)*x**5*y**2*z**2
    + (20)*x**4*y**3*z + (32)*x**3*y**4 + (3)*x**4*y*z**2 + (18)*x**3*y**2*z + (28)*x**2*y**3
    + (1)*x**3*z**2 + (8)*x**2*y*z + (16)*x*y**2 + (2)*x*z + (2)*y
)

F3 = (
    (R(3, 8))*x**6*y**4*z**2 + (3)*x**5*y**5*z + (6)*x**4*y**6 + (R(3, 2))*x**5*y**3*z**2
    + (R(21, 2))*x**4*y**4*z + (18)*x**3*y**5 + (R(9, 4))*x**4*y**2*z**2 + (14)*x**3*y**3*z
    + (R(43, 2))*x**2*y**4 + (R(3, 2))*x**3*y*z**2 + (9)*x**2*y**2*z + (14)*x*y**3
    + (R(3, 8))*x**2*z**2 + (3)*x*y*z + (R(9, 2))*y**2 + (R(1, 2))*z
)

F = [F1, F2, F3]

# --- generating data (provenance; see module docstring) --------------------
P_OF_W = (1)*w**3 + (-6)*w**2 + (6)*w**1
Q_OF_W = (R(3, 8))*w**4 + (-2)*w**3 + (R(3, 2))*w**2          # q(w) = int_0^w (s/2) p'(s) ds
G0 = 2
A  = -4
B  = -1
CLAIMED_DEGREE = 4          # = deg(p) + 1
CLAIMED_DETJ   = 2
