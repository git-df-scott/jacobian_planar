#!/usr/bin/env python3
"""Generator: emits self-contained runnable verification files, one per map."""
import sympy as sp
import sweep_lib as L
from sweep_lib import w, x, y, z
...
P_OF_W = {pw}
Q_OF_W = {qw}          # q(w) = int_0^w (s/2) p'(s) ds
G0 = {g0}
A  = {a}
B  = {b}
CLAIMED_DEGREE = {deg}          # = deg(p) + 1
CLAIMED_DETJ   = {detj}
...
def poly_src(f):
    """Explicit, readable sympy source for a polynomial."""
    p = sp.Poly(f, x, y, z)
...
def emit(fname, title, blurb, origin_note, pw, g0, a, b, deg, detj):
    F = L.build_map(pw, g0, a, b)
    src = TEMPLATE.format(
        title=title, blurb=blurb, origin_note=origin_note, fname=fname,
        F1=poly_src(F[0]), F2=poly_src(F[1]), F3=poly_src(F[2]),
        pw=uni_src(pw), qw=uni_src(L.q_from_p(pw)),
        g0=rat_src(g0), a=rat_src(a), b=rat_src(b),
        deg=deg, detj=rat_src(detj))
    open(fname, 'w').write(src)
    print("wrote", fname, " degrees",
          [sp.Poly(f, x, y, z).total_degree() for f in F])
