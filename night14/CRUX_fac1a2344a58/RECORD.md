# night14 CRUX_fac1a2344a58

Object (ring: Q[x,y]), family F2 -- F2 n=2 c=-1 a=1 h0=1 t=[(0, Fraction(1, 1))]

    P   = -1*x^2*y^2 - 1*x^2*y + 2*x*y^2 + (-1/4)*x^2 + 2*x*y - 1*y^2 + (1/2)*x + (1/4)
    P_x = -2*x*y^2 - 2*x*y + 2*y^2 + (-1/2)*x + 2*y + (1/2)
    P_y = -2*x^2*y - 1*x^2 + 4*x*y + 2*x - 2*y

total degree 4, deg_x 2, deg_y 2, 8 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='fac1a2344a58'][0]; \
