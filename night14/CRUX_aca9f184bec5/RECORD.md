# night14 CRUX_aca9f184bec5

Object (ring: Q[x,y]), family F2 -- F2 n=4 c=-2 a=0 h0=2 t=[(0, Fraction(2, 1)), (1, Fraction(3, 1)), (2, Fraction(3, 1))]

    P   = (-9/2)*x^8 - 9*x^7 - 6*x^6*y + (-21/2)*x^6 - 6*x^5*y - 2*x^4*y^2 - 6*x^5 - 4*x^4*y - 2*x^4 + 3*x^2 + 3*x + 2*y + 2
    P_x = -36*x^7 - 63*x^6 - 36*x^5*y - 63*x^5 - 30*x^4*y - 8*x^3*y^2 - 30*x^4 - 16*x^3*y - 8*x^3 + 6*x + 3
    P_y = -6*x^6 - 6*x^5 - 4*x^4*y - 4*x^4 + 2

total degree 8, deg_x 8, deg_y 2, 13 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='aca9f184bec5'][0]; \
