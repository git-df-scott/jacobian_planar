# night14 CRUX_13ac30c6e70c

Object (ring: Q[x,y]), family F1b -- F1b c1*x + c2*x^5*y^3 (+extra), top form has factors x and y

    P   = x^5*y^3 + 3*x
    P_x = 5*x^4*y^3 + 3
    P_y = 3*x^5*y^2

total degree 8, deg_x 5, deg_y 3, 2 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='13ac30c6e70c'][0]; \
