# night14 CRUX_c2005a8935aa

Object (ring: Q[x,y]), family F1b -- F1b c1*x + c2*x^5*y^4 (+extra), top form has factors x and y

    P   = -1*x^5*y^4 - 1*x^4*y^3 - 1*x
    P_x = -5*x^4*y^4 - 4*x^3*y^3 - 1
    P_y = -4*x^5*y^3 - 3*x^4*y^2

total degree 9, deg_x 5, deg_y 4, 3 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='c2005a8935aa'][0]; \
