# night14 CRUX_c59126bb0dbf

Object (ring: Q[x,y]), family F2b -- F2b n=2 c=-1 h0=1 d_n=-1 s=[(0, Fraction(2, 1))]

    P   = -1*x^2*y^2 + 2*x^2*y - 1*x^2 + y + (-5/4)
    P_x = -2*x*y^2 + 4*x*y - 2*x
    P_y = -2*x^2*y + 2*x^2 + 1
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='c59126bb0dbf'][0]; \
