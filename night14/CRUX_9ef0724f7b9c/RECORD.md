# night14 CRUX_9ef0724f7b9c

Object (ring: Q[x,y]), family F2 -- F2 n=3 c=-1 a=-1 h0=1 t=[(0, Fraction(-1, 1)), (1, Fraction(3, 1)), (2, Fraction(3, 1))]

    P   = (-9/4)*x^7 + (-45/4)*x^6 - 3*x^5*y - 21*x^5 - 12*x^4*y - 1*x^3*y^2 + (-33/2)*x^4 - 17*x^3*y - 3*x^2*y^2 + (-5/2)*x^3 - 9*x^2*y - 3*x*y^2 + (9/2)*x^2 - 1*y^2 + (9/4)*x + 2*y + (-3/4)
    P_x = (-63/4)*x^6 + (-135/2)*x^5 - 15*x^4*y - 105*x^4 - 48*x^3*y - 3*x^2*y^2 - 66*x^3 - 51*x^2*y - 6*x*y^2 + (-15/2)*x^2 - 18*x*y - 3*y^2 + 9*x + (9/4)
    P_y = -3*x^5 - 12*x^4 - 2*x^3*y - 17*x^3 - 6*x^2*y - 9*x^2 - 6*x*y - 2*y + 2

total degree 7, deg_x 7, deg_y 2, 17 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='9ef0724f7b9c'][0]; \
