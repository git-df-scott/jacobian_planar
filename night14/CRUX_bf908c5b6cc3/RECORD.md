# night14 CRUX_bf908c5b6cc3

Object (ring: Q[x,y]), family F2b -- F2b n=2 c=2 h0=2 d_n=-1 s=[(0, Fraction(2, 1))]

    P   = 2*x^2*y^2 + 2*x^2*y + (1/2)*x^2 + 2*y + (9/8)
    P_x = 4*x*y^2 + 4*x*y + x
    P_y = 4*x^2*y + 2*x^2 + 2
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='bf908c5b6cc3'][0]; \
