# night14 CRUX_b6a796932f30

Object (ring: Q[x,y]), family F3 -- F3 u+u^3*w, w=y+f(x) deg f=3

    P   = -3*x^6 + 2*x^5 - 3*x^4 + x^3*y + x
    P_x = -18*x^5 + 10*x^4 - 12*x^3 + 3*x^2*y + 1
    P_y = x^3

total degree 6, deg_x 6, deg_y 1, 5 terms.
...
## (b) SY-certificate -- gradient-row reduction over Q
    ...
    verdict NON_COORDINATE   nodes = 5  leaves = 2   0.0 s
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='b6a796932f30'][0]; \
