# night14 CRUX_164a86c4381d

Object (ring: Q[x,y]), family F3 -- F3 u+u^4*w, w=y+f(x) deg f=1

    P   = -3*x^5 + x^4*y + x^4 + x
    P_x = -15*x^4 + 4*x^3*y + 4*x^3 + 1
    P_y = x^4

total degree 5, deg_x 5, deg_y 1, 4 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='164a86c4381d'][0]; \
