# night14 CRUX_8944ddd9a60f

Object (ring: Q[x,y]), family F1b -- F1b c1*x + c2*x^3*y^1 (+extra), top form has factors x and y

    P   = -3*x^3*y + x
    P_x = -9*x^2*y + 1
    P_y = -3*x^3

total degree 4, deg_x 3, deg_y 1, 2 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='8944ddd9a60f'][0]; \
