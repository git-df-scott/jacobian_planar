# night14 CRUX_e21f12227b36

Object (ring: Q[x,y]), family F2 -- F2 n=2 c=2 a=-1 h0=-3 t=[(0, Fraction(2, 1))]

    P   = 2*x^2*y^2 + 4*x^2*y + 4*x*y^2 + 2*x^2 + 8*x*y + 2*y^2 + 4*x + y - 1
    P_x = 4*x*y^2 + 8*x*y + 4*y^2 + 4*x + 8*y + 4
    P_y = 4*x^2*y + 4*x^2 + 8*x*y + 8*x + 4*y + 1

total degree 4, deg_x 2, deg_y 2, 9 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='e21f12227b36'][0]; \
