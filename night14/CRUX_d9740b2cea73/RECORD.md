# night14 CRUX_d9740b2cea73

Object (ring: Q[x,y]), family F2 -- F2 n=4 c=-1 a=0 h0=-3 t=[(0, Fraction(-2, 1))]

    P   = -1*x^4*y^2 + 2*x^4*y - 1*x^4 - 3*y + 3
    P_x = -4*x^3*y^2 + 8*x^3*y - 4*x^3
    P_y = -2*x^4*y + 2*x^4 - 3
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='d9740b2cea73'][0]; \
