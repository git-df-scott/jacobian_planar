# night14 CRUX_0e7b40eabc35

Object (ring: Q[x,y]), family F2 -- F2 n=3 c=2 a=1 h0=2 t=[(0, Fraction(1, 1)), (1, Fraction(-2, 1))]

    P   = 2*x^5 - 4*x^4*y + 2*x^3*y^2 - 8*x^4 + 14*x^3*y - 6*x^2*y^2 + (25/2)*x^3 - 18*x^2*y + 6*x*y^2 + (-19/2)*x^2 + 10*x*y - 2*y^2 + (3/2)*x + (1/2)
    P_x = 10*x^4 - 16*x^3*y + 6*x^2*y^2 - 32*x^3 + 42*x^2*y - 12*x*y^2 + (75/2)*x^2 - 36*x*y + 6*y^2 - 19*x + 10*y + (3/2)
    P_y = -4*x^4 + 4*x^3*y + 14*x^3 - 12*x^2*y - 18*x^2 + 12*x*y + 10*x - 4*y
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='0e7b40eabc35'][0]; \
