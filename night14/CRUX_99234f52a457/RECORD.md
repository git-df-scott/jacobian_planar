# night14 CRUX_99234f52a457

Object (ring: Q[x,y]), family F2 -- F2 n=2 c=2 a=0 h0=1 t=[(0, Fraction(-3, 1)), (1, Fraction(-1, 1)), (2, Fraction(-3, 1))]

    P   = (9/2)*x^6 + 3*x^5 - 6*x^4*y + (19/2)*x^4 - 2*x^3*y + 2*x^2*y^2 + 3*x^3 - 6*x^2*y + 3*x^2 + (-1/2)*x + y + (-3/2)
    P_x = 27*x^5 + 15*x^4 - 24*x^3*y + 38*x^3 - 6*x^2*y + 4*x*y^2 + 9*x^2 - 12*x*y + 6*x + (-1/2)
    P_y = -6*x^4 - 2*x^3 + 4*x^2*y - 6*x^2 + 1

total degree 6, deg_x 6, deg_y 2, 12 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='99234f52a457'][0]; \
