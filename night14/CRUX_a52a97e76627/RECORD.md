# night14 CRUX_a52a97e76627

Object (ring: Q[x,y]), family F2b -- F2b n=1 c=-1 h0=1 d_n=3 s=[(0, Fraction(1, 1)), (1, Fraction(2, 1))]

    P   = -1*x^3 + 2*x^2*y - 1*x*y^2 - 1*x^2 + x*y + (-5/4)*x + y + (1/4)
    P_x = -3*x^2 + 4*x*y - 1*y^2 - 2*x + y + (-5/4)
    P_y = 2*x^2 - 2*x*y + x + 1

total degree 3, deg_x 3, deg_y 2, 8 terms.
...
## Independent corroboration of (b) -- FIB-screen, no SY involved
    ...
    (lambda, number of factors of P - lambda over Q, geometric genus if
     irreducible): [['0', 1, 0], ['1', 1, 0], ['-1', 1, 0], ['3/4', 2, None]]
    0.216 s
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='a52a97e76627'][0]; \
