# night14 CRUX_63a7aba444df

Object (ring: Q[x,y]), family F2b -- F2b n=5 c=1 h0=-1 d_n=-1 s=[(1, Fraction(3, 1)), (2, Fraction(3, 1))]

    P   = (9/4)*x^9 + (9/2)*x^8 + 3*x^7*y + (9/4)*x^7 + 3*x^6*y + x^5*y^2 + (-3/2)*x^2 + (-3/2)*x - 1*y + (1/4)
    P_x = (81/4)*x^8 + 36*x^7 + 21*x^6*y + (63/4)*x^6 + 18*x^5*y + 5*x^4*y^2 - 3*x + (-3/2)
    P_y = 3*x^7 + 3*x^6 + 2*x^5*y - 1

total degree 9, deg_x 9, deg_y 2, 10 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='63a7aba444df'][0]; \
