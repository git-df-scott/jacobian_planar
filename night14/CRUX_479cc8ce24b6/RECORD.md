# night14 CRUX_479cc8ce24b6

Object (ring: Q[x,y]), family F2 -- F2 n=4 c=1 a=2 h0=1 t=[(0, Fraction(1, 1))]

    P   = x^4*y^2 + x^4*y - 8*x^3*y^2 + (1/4)*x^4 - 8*x^3*y + 24*x^2*y^2 - 2*x^3 + 24*x^2*y - 32*x*y^2 + 6*x^2 - 32*x*y + 16*y^2 - 8*x + 17*y + (9/2)
    P_x = 4*x^3*y^2 + 4*x^3*y - 24*x^2*y^2 + x^3 - 24*x^2*y + 48*x*y^2 - 6*x^2 + 48*x*y - 32*y^2 + 12*x - 32*y - 8
    P_y = 2*x^4*y + x^4 - 16*x^3*y - 8*x^3 + 48*x^2*y + 24*x^2 - 64*x*y - 32*x + 32*y + 17
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='479cc8ce24b6'][0]; \
