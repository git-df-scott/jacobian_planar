# night14 CRUX_c0b6089f980a

Object (ring: Q[x,y]), family F2 -- F2 n=3 c=1 a=0 h0=1 t=[(0, Fraction(-1, 1))]

    P   = x^3*y^2 - 1*x^3*y + (1/4)*x^3 + y + (-1/2)
    P_x = 3*x^2*y^2 - 3*x^2*y + (3/4)*x^2
    P_y = 2*x^3*y - 1*x^3 + 1
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='c0b6089f980a'][0]; \
