# night14 CRUX_223f71c4db9d

Object (ring: Q[x,y]), family F1b -- F1b c1*x + c2*x^4*y^4 (+extra), top form has factors x and y

    P   = -3*x^4*y^4 + 2*x
    P_x = -12*x^3*y^4 + 2
    P_y = -12*x^4*y^3

total degree 8, deg_x 4, deg_y 4, 2 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='223f71c4db9d'][0]; \
