# night14 CRUX_e34dd8dbd66b

Object (ring: Q[x,y]), family F2b -- F2b n=2 c=2 h0=1 d_n=3 s=[(0, Fraction(-3, 1))]

    P   = 2*x^2*y^2 - 3*x^2*y + (9/8)*x^2 + y + (-9/8)
    P_x = 4*x*y^2 - 6*x*y + (9/4)*x
    P_y = 4*x^2*y - 3*x^2 + 1

total degree 4, deg_x 2, deg_y 2, 5 terms.
...
## Independent corroboration of (b) -- FIB-screen, no SY involved
    ...
    (lambda, number of factors of P - lambda over Q, geometric genus if
     irreducible): [['0', 1, 0], ['1', 1, 0], ['-1', 1, 0], ['-3/8', 2, None]]
    0.21 s
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='e34dd8dbd66b'][0]; \
