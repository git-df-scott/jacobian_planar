# night14 CRUX_a22d19d84467

Object (ring: Q[x,y]), family F3 -- F3 u+u^4*w, w=y+f(x) deg f=2

    P   = -1*x^6 - 2*x^5 + x^4*y - 1*x^4 + x
    P_x = -6*x^5 - 10*x^4 + 4*x^3*y - 4*x^3 + 1
    P_y = x^4
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='a22d19d84467'][0]; \
