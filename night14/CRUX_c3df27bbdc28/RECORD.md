# night14 CRUX_c3df27bbdc28

Object (ring: Q[x,y]), family F3 -- F3 u+u^3*w, w=y+f(x) deg f=0

    P   = x^3*y + 2*x^3 + x
    P_x = 3*x^2*y + 6*x^2 + 1
    P_y = x^3

total degree 4, deg_x 3, deg_y 1, 3 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='c3df27bbdc28'][0]; \
