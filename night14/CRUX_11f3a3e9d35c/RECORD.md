# night14 CRUX_11f3a3e9d35c

Object (ring: Q[x,y]), family F2b -- F2b n=1 c=2 h0=1 d_n=-1 s=[(1, Fraction(-2, 1))]

    P   = (1/2)*x^3 - 2*x^2*y + 2*x*y^2 + (-1/2)*x + y + (1/8)
    P_x = (3/2)*x^2 - 4*x*y + 2*y^2 + (-1/2)
    P_y = -2*x^2 + 4*x*y + 1

total degree 3, deg_x 3, deg_y 2, 6 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='11f3a3e9d35c'][0]; \
