# night14 CRUX_8afa03e98aa2

Object (ring: Q[x,y]), family F2b -- F2b n=4 c=1 h0=2 d_n=1 s=[(0, Fraction(1, 1))]

    P   = x^4*y^2 + x^4*y + (1/4)*x^4 + 2*y + (3/4)
    P_x = 4*x^3*y^2 + 4*x^3*y + x^3
    P_y = 2*x^4*y + x^4 + 2
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='8afa03e98aa2'][0]; \
