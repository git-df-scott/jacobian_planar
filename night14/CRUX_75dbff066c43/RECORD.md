# night14 CRUX_75dbff066c43

Object (ring: Q[x,y]), family F2b -- F2b n=2 c=-1 h0=-1 d_n=-2 s=[(0, Fraction(-3, 1))]

    P   = -1*x^2*y^2 - 3*x^2*y + (-9/4)*x^2 - 1*y - 2
    P_x = -2*x*y^2 - 6*x*y + (-9/2)*x
    P_y = -2*x^2*y - 3*x^2 - 1
...
## Independent corroboration of (b) -- FIB-screen, no SY involved
    ...
    (lambda, number of factors of P - lambda over Q, geometric genus if
     irreducible): [['0', 1, 0], ['1', 1, 0], ['-1', 1, 0], ['-1/2', 2, None]]
    0.214 s
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='75dbff066c43'][0]; \
