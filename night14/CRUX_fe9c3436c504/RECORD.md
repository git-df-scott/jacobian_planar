# night14 CRUX_fe9c3436c504

Object (ring: Q[x,y]), family F2b -- F2b n=1 c=1 h0=1 d_n=3 s=[(0, Fraction(1, 1))]

    P   = x*y^2 + x*y + (1/4)*x + y + (-1/4)
    P_x = y^2 + y + (1/4)
    P_y = 2*x*y + x + 1

total degree 3, deg_x 1, deg_y 2, 5 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='fe9c3436c504'][0]; \
