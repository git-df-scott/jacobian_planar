# night14 CRUX_8486fde049ff

Object (ring: Q[x,y]), family F2 -- F2 n=2 c=-2 a=0 h0=-1 t=[(0, Fraction(1, 1))]

    P   = -2*x^2*y^2 - 2*x^2*y + (-1/2)*x^2 - 1*y + (-1/2)
    P_x = -4*x*y^2 - 4*x*y - 1*x
    P_y = -4*x^2*y - 2*x^2 - 1
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='8486fde049ff'][0]; \
