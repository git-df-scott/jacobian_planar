# night14 CRUX_4f443eee78bd

Object (ring: Q[x,y]), family F2 -- F2 n=1 c=-2 a=-1 h0=1 t=[(0, Fraction(2, 1))]

    P   = -2*x*y^2 - 4*x*y - 2*y^2 - 2*x - 3*y - 1
    P_x = -2*y^2 - 4*y - 2
    P_y = -4*x*y - 4*x - 4*y - 3

total degree 3, deg_x 1, deg_y 2, 6 terms.
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='4f443eee78bd'][0]; \
