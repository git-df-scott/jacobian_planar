# night14 CRUX_d28efa65a045

Object (ring: Q[x,y]), family F2b -- F2b n=5 c=1 h0=2 d_n=3 s=[(0, Fraction(1, 1)), (1, Fraction(3, 1))]

    P   = (9/4)*x^7 + 3*x^6*y + x^5*y^2 + (3/2)*x^6 + x^5*y + (1/4)*x^5 + 3*x + 2*y + (1/4)
    P_x = (63/4)*x^6 + 18*x^5*y + 5*x^4*y^2 + 9*x^5 + 5*x^4*y + (5/4)*x^4 + 3
    P_y = 3*x^6 + 2*x^5*y + x^5 + 2
...
## Independent corroboration of (b) -- FIB-screen, no SY involved
    ...
    (lambda, number of factors of P - lambda over Q, geometric genus if
     irreducible): [['0', 1, 2], ['1', 1, 2], ['-1', 1, 2], ['-3/4', 2, None]]
    0.214 s
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='d28efa65a045'][0]; \
