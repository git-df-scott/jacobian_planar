# night14 CRUX_fe940047aaa5

Object (ring: Q[x,y]), family F2b -- F2b n=5 c=1 h0=2 d_n=-2 s=[(0, Fraction(1, 1))]

    P   = x^5*y^2 + x^5*y + (1/4)*x^5 + 2*y + (3/2)
    P_x = 5*x^4*y^2 + 5*x^4*y + (5/4)*x^4
    P_y = 2*x^5*y + x^5 + 2
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='fe940047aaa5'][0]; \
