# night14 CRUX_68605c498ba1

Object (ring: Q[x,y]), family F2b -- F2b n=5 c=-1 h0=1 d_n=-2 s=[(0, Fraction(3, 1))]

    P   = -1*x^5*y^2 + 3*x^5*y + (-9/4)*x^5 + y - 2
    P_x = -5*x^4*y^2 + 15*x^4*y + (-45/4)*x^4
    P_y = -2*x^5*y + 3*x^5 + 1
...
## Reproduce
    ...
    cd night14 && python3 -c "import json, poly14 as P, sy14, utest14, fib14; \
      r = [r for r in json.load(open('records.json')) if r['hash']=='68605c498ba1'][0]; \
