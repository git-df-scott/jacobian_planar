"""COPY of campaign/audit_tracks/trackD_twoprime.py, unmodified except for the
state-file path, so the campaign's own state file is never written to.
...
STATE = os.path.join(HERE, "h2_state.json")
TARGETS = os.path.join(HERE, "trackD_targets_priority.json")
