...
def main():
    ...
    ver = load("verify_certs.json", None)
    if ver:
        w("## 6b. Independent re-verification of the emitted certificates")
        w()
        w("`verify_certs.py` trusts nothing in the records but the raw data: it looks")
        w("each `P` up in the pools by hash, rebuilds the carrier and the Keller")
        w("system from scratch, and re-checks each certificate over `Q` with")
        w("`Fraction` arithmetic without calling `exact.decide`.")
        w()
        w("| arm | certificate | result | count |")
        w("| --- | --- | --- | --- |")
        for k in sorted(ver["tally"]):
            a, c, r = [t.strip() for t in k.split("|")]
            w("| %s | %s | %s | %d |" % (a, c, r, ver["tally"][k]))
        w()
        w("**Failures: %d.**" % len(ver["failures"]))
        for f in ver["failures"]:
            w("- %s" % (f,))
        w()
        w("`rank_full_column_exact` is marked `not_recheckable` because it is a rank")
        w("statement at the scheduling prime rather than an object that can be")
        w("re-derived from the record; its validity is the lower-bound argument in")
        w("`MATE_V1.md` (E2), not a stored witness. Every certificate that HAS a")
        w("stored witness -- all 65 lambda vectors and all 20 exact solutions -- was")
        w("re-derived and re-checked here and holds.")
        w()
    ...
    w()
    w("**(F6) the lambda certificates were not actually recorded.** A stage record")
    w("kept `lambda_support`, the number of nonzero entries, but not the vector, so")
    w("the certificate the brief asks to be emitted on EMPTY could not be checked")
    w("from the record. `exact.py` now stores `lambda_vector` (entries")
    w("`[[i,j],[num,den]]`) and a `lambda_reverified` flag; recording only, no")
    w("verdict depends on it. `verify_certs.py` (section 6b) re-checks them all.")
    w()
    w("**(F7) the carrier thinning compounds its strides.** Both v1 carrier builders")
    w("thin by repeatedly filtering the ALREADY thinned set, so `t = 2` followed by")
    w("`t = 3` keeps only the points divisible by 6 -- about 1/36 of the set -- while")
    w("recording `thin_t = 3`. `n_used` is accurate either way and is what every")
    w("verdict in this file is relative to, so no verdict here is affected; but the")
    w("recorded `thin_t` understates how much was removed, and the carriers are")
    w("smaller than the cap intends. Recorded, not changed in v1 (changing it would")
    w("invalidate the run this file reports); v2's `_thin` recomputes from the base")
    w("set instead, and `MATE_V2.md` uses that.")
    w()
