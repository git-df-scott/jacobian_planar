...
The consolidated forward plan lives in [`../FRAMEWORK.md`](../FRAMEWORK.md),
which covers the whole campaign (Sessions 1–35) in one document. In short, and
in order of expected value:

1. **Close geometric degree 2** by finishing the Cremona classification — kill
   the Geiser and Bertini involutions. The Session-33 fixed-point lemma plus
   "a positive-genus curve is not a line" appears to do it; the work is ruling
   out the normalized fixed curve hiding over an indeterminacy point. If it
   lands, `d = 2` is closed outright.
2. **Push the `slimgb` computation further** — the `(3,n)` slice and the
   `h`-branch past coefficient degree 5. The binding constraint is *memory*, so
   a larger machine buys degrees directly; msolve (F4 + FGLM) is the next engine
   tier. Reduce variables before the Gröbner call by substituting the linear
   equations and the Session-31 ladder `h ∣ q`.
3. **Attack `d ≥ 3` through the monodromy** — bound the components of `S_F`,
   hence the meridian generators of `π₁(C² ∖ S_F)`, hence the transitive
   subgroups of `S_d` that can occur; combine with `χ(F⁻¹(S_F)) ≡ 1 (mod d)`.
4. Apply the `χ` congruence as a free filter to any candidate anyone proposes.

Do **not** spend time on: Mathieu–Zhao (implies the JC), pseudo-holomorphic
curves (the taming hypothesis fails identically — Session 34), a plane analogue
of the tangent sweep (closed at every order), the cusp-chain framework family
(closed), or brute-force search above degree 108 (11,990 unknowns).

Older items still standing from the framework side:

- Re-referee the pole-fiber theorem (Session 13/14, Theorem 3) — it is the
  single load-bearing hypothesis, and Session 19's finding 2 says exactly how
  much slack would break it (one unit of pole order at `U = 0`).
- Redo the `D = 4` admissibility test for the Second Framework's profile and
