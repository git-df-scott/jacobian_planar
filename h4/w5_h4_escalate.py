...
# The four cells Session 35 recorded as out of memory, plus the next rungs of
# the same ladder, so the escalation continues rather than stopping where it was
# staged.  Each cell is run at TWO compliant primes (both == 1 mod 3), and the
# two verdicts must agree; a disagreement is reported, never averaged.
OOM_CELLS = [("hbranch", 4, "t", 6), ("hbranch", 5, "t", 4),
             ("hbranch", 5, "t", 5), ("hbranch", 6, "t", 4),
             ("hbranch", 4, "t", 7), ("hbranch", 5, "t", 6),
             ("hbranch", 6, "t", 5), ("hbranch", 7, "t", 4)]
...
for kind, K, hs, D in OOM_CELLS:
    ...
    N = nfree + 2
    per = {}
    for PP in (65521, 65539):
        assert PP % 3 == 1
        code2 = hbranch_code(K, hs, D, p=PP)
        gg2 = dump_generators(code2, budget=1200) if PP != P0 else gg
        if not gg2:
            per[PP] = "BUILD-FAIL"
            continue
        ms = to_msolve(gg2, N, PP, os.path.join(SCRATCH, f"h_{K}_{D}_{PP}.ms"))
        v, secs, det = run_msolve(ms, budget=int(os.environ.get("W2_MSTMO", 1800)))
        per[PP] = v
        cell = f"k={K} h={hs} deg<={D} p={PP}"
        print(f"  {cell:26s} {len(gg2):>5d} {v:>10s} {secs:>8.1f}", flush=True)
    vs = sorted(set(per.values()))
    v = per.get(65521, "UNKNOWN")
    if len(vs) > 1 and "EMPTY" in vs and "NONEMPTY" in vs:
        v = "DISAGREE"
    elif per.get(65521) == per.get(65539):
        v = per.get(65521)
    res[f"k={K},{hs},{D}"] = v
    print(f"    -> combined k={K} deg<={D}: {v}   {per}", flush=True)
