...
for d in range(18,6,-1):
    ...
    for g in gates:
        fs=strip(g)
        if len(fs)==1:
            forced.append(fs[0][0]); print(f"d={d}: FORCED (power mult {fs[0][1]}): {fs[0][0]}",flush=True)
        elif len(fs)>1:
            print(f"d={d}: REDUCIBLE gate (BRANCH): {[str(f) for f,_ in fs]}",flush=True)
            forced.append(None)
    if any(f is None for f in forced):
        log.append((d,'branch',gates)); print("   -> stopping at first branch point"); break
    # impose forced conditions, then solve the rung
    for f in forced:
        vs=[s for s in f.free_symbols if s not in NONZERO]
        if not vs: continue
    ...
    if not s:
        print(f"d={d}: still inconsistent after imposing -> extra gates"); log.append((d,'stuck',cs)); break
    ...
    res=rung(d,sub)
    fr=[n for n in new if n not in sub]; free_params+=fr
    print(f"d={d}: solved {len(s[0])}, free {len(fr)}, residual {len(res)}",flush=True)
    log.append((d,'ok',forced))
